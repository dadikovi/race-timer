...
func TestPostGroupsWithValidData(t *testing.T) {
	...
	assert.Equal(t, segmentId, responseBody["segmentId"], "Should return the given segment name")
	...
	assert.Equal(t, segmentId, groupsFromDatabase[0]["segment_id"])
	...
	assert.Equal(t, expectedGroupId, racesFromDatabase[0]["active_group_id"])
	assert.Equal(t, int64(1), racesFromDatabase[0]["id"])
}

func getGroups() []RAWROW {
	rows, _ := a.DB.Query("SELECT * FROM groups")
	defer rows.Close()
	var result []RAWROW
...
func getRaces() []RAWROW {
	rows, _ := a.DB.Query("SELECT * FROM races")
	defer rows.Close()
	var result []RAWROW
